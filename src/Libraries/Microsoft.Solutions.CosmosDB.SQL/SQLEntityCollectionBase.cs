...
namespace Microsoft.Solutions.CosmosDB.SQL {
    public class SQLEntityCollectionBase<TEntity> : IDataRepositoryProvider<TEntity>
        ...
        public SQLEntityCollectionBase(string DataConnectionString, string CollectionName, string ContainerName = "", Func<CosmosClient, string, string, string, IRepository<TEntity, string>> repoFactory = null) {
            CosmosClientManager.DataconnectionString = DataConnectionString;
            CosmosClient _client = CosmosClientManager.Instance;

            this.EntityCollection = repoFactory?.Invoke(_client, DataConnectionString, CollectionName, ContainerName);

            this.EntityCollection ??= new BusinessTransactionRepository<TEntity, string>(_client, CollectionName, ContainerName);
        }
    }
